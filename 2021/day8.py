...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec  7 11:08:21 2021
...
# We label them differently from AOC
"""
. 00 .
1    3
1    3
...
for line in _input:
    # d keeps track of the letters in each digit
    d = defaultdict(set)
    ...
    # Get all codes in the row
    line = line.strip().replace('| ', '')
    codes = line.split(' ')
    code_lengths = set(len(x) for x in codes)
    # Find a two-digit code -- it's 1
    d[1] = set([x for x in codes if len(x) == 2][0])
    # Find a three-digit code -- it's 7
    d[7] = set([x for x in codes if len(x) == 3][0])
    # Find a four-digit code -- it's 4
    d[4] = set([x for x in codes if len(x) == 4][0])
    # Find a three-digit code
    d[8] = set([x for x in codes if len(x) == 7][0])
    
    # The top bar is in 7 but not 1
    set_top = set_7.difference(d[1])
    
    ## Length 6 codons
    # "6" has length 6 and does not include both bars from "1"
    d[6] = set([x for x in codes if len(x) == 6 and len(set(x).intersection(d[1])) == 1][0])
    # "9" has length 6 and includes all the bars from "4"
    d[9] = set([x for x in codes if len(x) == 6 and len(set(x).intersection(d[4])) == 4][0])
    # "0" is the other one of length 6
    d[0] = set([x for x in codes if len(x) == 6 and set(x) != d[6] and set(x) != d[9]][0])
    
    ## Length 5 codons
    # "3" contains both codons from "1"
    d[3] = set([x for x in codes if len(x) == 5 and len(set(x).intersection(d[1])) == 2][0])
    # "5" contains all the codons from "6"
    d[5] = set([x for x in codes if len(x) == 5 and len(set(x).intersection(d[6])) == 5][0])
    # "2" is the remaining one
    d[2] = set([x for x in codes if len(x) == 5 and set(x) != d[3] and set(x) != d[5]][0])
    
    # Now process the output and get a number from it
    s = ''
    for code in o:
        for k in d.keys():
            if set(code) == d[k]:
