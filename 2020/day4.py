...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec  4 09:10:59 2020
...
eyr:2039 hgt:64
ecl:#ab45a8 byr:2009
iyr:2025 pid:182cm hcl:d1614a cid:103'''.strip()
...
def is_valid_passport_dict(d):
    return set(d.keys()).issuperset(required_fields)
    
ct = 0
for passport in passports:
    d = split_passport_to_dict(passport)
    if is_valid_passport_dict(d):
        ct += 1
print(ct)
...
def is_valid_passport_dict2(d):
    if not is_valid_passport_dict(d):
        return False
    # byr
    try:
        if int(d['byr']) > 2002 or int(d['byr']) < 1920:
            return False
    except:
        return False
    # iyr
    try:
        if int(d['iyr']) > 2020 or int(d['iyr']) < 2010:
            return False
    except:
        return False
    # eyr
    try:
        if int(d['eyr']) > 2030 or int(d['eyr']) < 2020:
            return False
    except:
        return False
    # hgt
    hgt1 = re.match(r'(\d\d\d)cm$', d['hgt'])
    hgt2 = re.match(r'(\d\d)in$', d['hgt'])
    if hgt1 is not None:
        height = hgt1.groups()[0]
        if int(height) < 150 or int(height) > 193:
            return False
    elif hgt2 is not None:
        height = hgt2.groups()[0]
        if int(height) < 59 or int(height) > 76:
            return False
    else:
        return False
    # hcl
    hcl = re.match(r'\#[0-9a-f]{6}$', d['hcl'])
    if hcl is None:
        return False
    # ecl
